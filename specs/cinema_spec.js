...
describe('Cinema', function () {
  ...
  it('should be able to check whether there are some films from a particular year', function () {
    const actual = cinema.yearCheck(2017);
    assert.deepStrictEqual(actual, true);
  });

  it('should be able to check whether there are no films from a particular year', function () {
    const actual = cinema.yearCheck(1938);
    assert.strictEqual(actual, false);
  });

  it('should be able to check whether all films are over a particular length', function () {
    const actual = cinema.lengthCheck(115);
    assert.strictEqual(actual, false);
  });

  it('should be able to calculate total running time of all films', function () {
    const actual = cinema.runningTotal();
  ...
  it('should be able to filter films by property', function() {
    const actual = cinema.filmsByProperty('year', 1938);
    assert.deepStrictEqual(actual, 'There are no films matching that year!');
  });
